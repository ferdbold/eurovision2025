...
export default function PerformanceIntro(props: IPerformanceIntroProps) {
	return (
		<div key={props.performance.id} className="w-screen h-screen relative bg-radial-[at_50%_100%] from-rose-700 from-5% to-purple-700/25 animate-fade">
			...
			<p className="absolute top-16 left-16 font-fancy text-[160px] animate-fade-right animate-ease-out">#{props.performance.id}</p>
			...
			<div className="absolute w-full bottom-0 animate-fade-left animate-ease-out animate-delay-2000">
				<p className="text-shadow-lg text-shadow-black text-[120px] font-fancy mr-24 -mb-13 text-right">{props.performance.performer}</p>
				<div className="bg-black/50 p-4">
					<div className="flex flex-row w-full items-center justify-between pr-20">
						<div className="flex flex-row items-center gap-2">
							<img className="relative -top-8 size-48 -my-8 -mr-4" src={props.performance.flag_url} alt={props.performance.country} />
							<div className="text-4xl">{props.performance.country}, {props.performance.year}</div>
						</div>
						<div className="text-right">
							<p className="text-6xl font-bold">{props.performance.song}</p>
							<p className="text-3xl">{props.performance.artist}</p>
						</div>
					</div>
				</div>
			</div>
